...
cv_tron <- circle_perimeter(3.5)
cat("Chu vi hình tròn:", cv_tron, "\n")
...
is_even <- function(n) {
  if (n %% 1 != 0) stop("Input phải là số nguyên")
  if (n %% 2 == 0) "Chẵn" else "Lẻ"}
...
#   - Loại bỏ giá trị NA
calc_mean_score <- function(scores) {
  # Loại bỏ NA
  scores <- na.omit(scores)
  
  # Kiểm tra miền giá trị 0-10
  if (any(scores < 0 | scores > 10)) {
    stop("Điểm số phải nằm trong khoảng từ 0 đến 10")
  }
...
find_primes <- function(n) {
  ...
  # Tận dụng hàm is_prime đã có trong bài học
  primes <- Filter(is_prime, 1:n)
  return(primes)}
...
pascal_triangle <- function(n) {
  for (i in 0:(n-1)) {
    row <- sapply(0:i, function(k) {
      choose(i, k) # choose là hàm tính tổ hợp có sẵn trong R
    })
    cat(rep("  ", n - i)) # Tạo khoảng cách để tam giác cân đối
    cat(row, "\n")  }}
...
classify_student <- function(score) {
  if (score < 0 || score > 10) stop("Điểm không hợp lệ")
  
  res <- list()
  if (score >= 8.5) {
    res$rank <- "Xuất sắc"; res$gpa <- 4.0
  } else if (score >= 7.0) {
    res$rank <- "Khá/Giỏi"; res$gpa <- 3.0
  } else if (score >= 5.5) {
    res$rank <- "Trung bình khá"; res$gpa <- 2.0
  } else if (score >= 4.0) {
    res$rank <- "Trung bình"; res$gpa <- 1.0
  } else {
    res$rank <- "Yếu"; res$gpa <- 0.0
  }
  return(res)}
# ------------------------------------------------------------------------------
